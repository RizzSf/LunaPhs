...
if not Success then
    warn("\nChickHook Library - Failed to load Feather Icons. Error code: " .. Response .. "\n")
end
...
local ChickHook = Instance.new("ScreenGui")
ChickHook.Name = "ChickHook"
if syn then
    syn.protect_gui(ChickHook)
    ChickHook.Parent = game.CoreGui
else
    ChickHook.Parent = gethui() or game.CoreGui
end

if gethui then
    for _, Interface in ipairs(gethui():GetChildren()) do
        if Interface.Name == ChickHook.Name and Interface ~= ChickHook then
            Interface:Destroy()
        end
    end
else
...
function ChickHookLib:IsRunning()
    if gethui then
        return ChickHook.Parent == gethui()
    else
        return ChickHook.Parent == game:GetService("CoreGui")
    end
end

local function CreateConnection(Signal, Function)
    if (not ChickHookLib:IsRunning()) then
        return
    end
    local SignalConnect = Signal:Connect(Function)
    table.insert(ChickHookLib.Connections, SignalConnect)
    return SignalConnect
end

task.spawn(
    function()
        while (ChickHookLib:IsRunning()) do
            wait()
        end

        for _, Connection in next, ChickHookLib.Connections do
            Connection:Disconnect()
        end
    end
)

local function CreateDraggingFunctionality(DragPoint, Main)
    pcall(
        function()
...
local function CreateElement(ElementName, ElementFunction)
    ChickHookLib.Elements[ElementName] = function(...)
        return ElementFunction(...)
    end
end

local function MakeElement(ElementName, ...)
    local NewElement = ChickHookLib.Elements[ElementName](...)
    return NewElement
end
...
local function CreateThemeObject(Object, Type)
    if not ChickHookLib.ThemeObjects[Type] then
        ChickHookLib.ThemeObjects[Type] = {}
    end
    table.insert(ChickHookLib.ThemeObjects[Type], Object)
    Object[ReturnProperty(Object)] = ChickHookLib.Themes[ChickHookLib.SelectedTheme][Type]
    return Object
end

local function SetTheme()
    for Name, Type in pairs(ChickHookLib.ThemeObjects) do
        for _, Object in pairs(Type) do
            Object[ReturnProperty(Object)] = ChickHookLib.Themes[ChickHookLib.SelectedTheme][Name]
        end
    end
end
...
local function LoadCfg(Config)
    ...
    table.foreach(
        Data,
        function(a, b)
            if ChickHookLib.Flags[a] then
                spawn(
                    function()
                        if ChickHookLib.Flags[a].Type == "Colorpicker" then
                            ChickHookLib.Flags[a]:Set(UnpackColor(b))
                        else
                            ChickHookLib.Flags[a]:Set(b)
                        end
                    end
                )
            else
                warn("ChickHook Library Config Loader - Could not find ", a, b)
            end
        end
    )
end
...
local function SaveCfg(Name)
    local Data = {}
    for i, v in pairs(ChickHookLib.Flags) do
        if v.Save then
    ...
    writefile(ChickHookLib.Folder .. "/" .. Name .. ".txt", tostring(HttpService:JSONEncode(Data)))
end
    ...
    function(Scale, Offset)
        local List =
            ...
            "UIListLayout",
            {
                ...
                PCreateing = UDim.new(Scale or 0, Offset or 0)
            }
        )
        return List
    end
...
CreateElement(
    ...
    function(Bottom, Left, Right, Top)
        local PCreateing =
            ...
            "UIPCreateing",
            {
                PCreateingBottom = UDim.new(0, Bottom or 4),
                PCreateingLeft = UDim.new(0, Left or 4),
                PCreateingRight = UDim.new(0, Right or 4),
                PCreateingTop = UDim.new(0, Top or 4)
            }
        )
        return PCreateing
    end
...
function ChickHookLib:MakeNotification(NotificationConfig)
    spawn(
        function()
            ...
            local NotificationFrame =
                ...
                ),
                {
                    ...
                    MakeElement("PCreateing", 12, 12, 12, 12),
                    SetProps(
...
function ChickHookLib:Init()
    if ChickHookLib.SaveCfg then
        pcall(
            function()
                if isfile(ChickHookLib.Folder .. "/" .. game.GameId .. ".txt") then
                    LoadCfg(readfile(ChickHookLib.Folder .. "/" .. game.GameId .. ".txt"))
                    ChickHookLib:MakeNotification(
                        {
                            Name = "Configuration",
...
function ChickHookLib:MakeWindow(WindowConfig)
    local FirstTab = true
    ...
    WindowConfig.Name = WindowConfig.Name or "ChickHook Library"
    ...
    WindowConfig.IntroText = WindowConfig.IntroText or "ChickHook Library"
    WindowConfig.CloseCallback = WindowConfig.CloseCallback or function()
        end
    WindowConfig.ShowIcon = WindowConfig.ShowIcon or false
    WindowConfig.Icon = WindowConfig.Icon or "rbxassetid://8834748103"
    WindowConfig.IntroIcon = WindowConfig.IntroIcon or "rbxassetid://8834748103"
    ChickHookLib.Folder = WindowConfig.ConfigFolder
    ChickHookLib.SaveCfg = WindowConfig.SaveConfig
    ...
    local TabHolder =
        ...
        SetChildren(
            ...
            {
                MakeElement("List"),
                MakeElement("PCreateing", 8, 0, 0, 8)
            }
        ),
        "Divider"
    )
                ...
                SetChildren(
                    ...
                    ),
                    {
                        ...
                        SetChildren(
                            ...
                            {
                                CreateThemeObject(MakeElement("Stroke"), "Stroke"),
                                MakeElement("Corner", 1)
                            }
                        ),
                        CreateThemeObject(
    ...
    CreateDraggingFunctionality(DragPoint, MainWindow)
    ...
    function TabFunction:MakeTab(TabConfig)
        ...
        local Container =
            ...
            SetChildren(
                ...
                ),
                {
                    ...
                    MakeElement("PCreateing", 15, 10, 10, 15)
                }
            ),
            "Divider"
        )
        ...
        local function GetElements(ItemParent)
                        ...
                        TweenService:Create(
                            ...
                            TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
                            {
                                BackgroundColor3 = Color3.fromRGB(
                                    ChickHookLib.Themes[ChickHookLib.SelectedTheme].Second.R * 255 + 3,
                                    ChickHookLib.Themes[ChickHookLib.SelectedTheme].Second.G * 255 + 3,
                                    ChickHookLib.Themes[ChickHookLib.SelectedTheme].Second.B * 255 + 3
                                )
                            }
                        ):Play()
                    end
                ...
                CreateConnection(
                    ...
                    function()
                        TweenService:Create(
                            ButtonFrame,
                            TweenInfo.new(0.25, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
                            {BackgroundColor3 = ChickHookLib.Themes[ChickHookLib.SelectedTheme].Second}
                        ):Play()
                    end
                )
                ...
                CreateConnection(
                    ...
                    function()
                        TweenService:Create(
                            ...
                            {
                                BackgroundColor3 = Color3.fromRGB(
                                    ChickHookLib.Themes[ChickHookLib.SelectedTheme].Second.R * 255 + 6,
                                    ChickHookLib.Themes[ChickHookLib.SelectedTheme].Second.G * 255 + 6,
                                    ChickHookLib.Themes[ChickHookLib.SelectedTheme].Second.B * 255 + 6
                                )
                            }
                        ):Play()
                    end
            ...
            function ElementFunction:CreateToggle(ToggleConfig)
                ...
                function Toggle:Set(Value)
                    ...
                    TweenService:Create(
                        ...
                        {BackgroundColor3 = Toggle.Value and ToggleConfig.Color or ChickHookLib.Themes.Default.Divider}
                    ):Play()
                    TweenService:Create(
                        ToggleBox.Stroke,
                        TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
                        {Color = Toggle.Value and ToggleConfig.Color or ChickHookLib.Themes.Default.Stroke}
                    ):Play()
                    TweenService:Create(
                        ToggleBox.Ico,
                        TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.Out),
                ...
                if ToggleConfig.Flag then
                    ChickHookLib.Flags[ToggleConfig.Flag] = Toggle
                end
                return Toggle
            end
            function ElementFunction:CreateSlider(SliderConfig)
                ...
                if SliderConfig.Flag then
                    ChickHookLib.Flags[SliderConfig.Flag] = Slider
                end
                return Slider
            end
            function ElementFunction:CreateDropdown(DropdownConfig)
                ...
                if DropdownConfig.Flag then
                    ChickHookLib.Flags[DropdownConfig.Flag] = Dropdown
                end
                return Dropdown
            end
            function ElementFunction:CreateBind(BindConfig)
                ...
                if BindConfig.Flag then
                    ChickHookLib.Flags[BindConfig.Flag] = Bind
                end
                return Bind
            end
            ...
            function ElementFunction:CreateColorpicker(ColorpickerConfig)
                ...
                local ColorpickerContainer =
                    ...
                    },
                    {
                        ...
                        Create(
                            "UIPCreateing",
                            {
                                PCreateingLeft = UDim.new(0, 35),
                                PCreateingRight = UDim.new(0, 35),
                                PCreateingBottom = UDim.new(0, 10),
                                PCreateingTop = UDim.new(0, 17)
                            }
                        )
                    }
                )
                ...
                if ColorpickerConfig.Flag then
                    ChickHookLib.Flags[ColorpickerConfig.Flag] = Colorpicker
                end
                return Colorpicker
            end
            return ElementFunction
        ...
        if TabConfig.PremiumOnly then
            ...
            Container:FindFirstChild("UIPCreateing"):Destroy()
            SetChildren(
                SetProps(
                    MakeElement("TFrame"),
                    {
    ...
    ChickHookLib:MakeNotification(
        {
            Name = "ChickHook Library",
            Content = "New library! Support server [discord.gg/JCmQc2ZYwJ]",
            Time = 5
        }
    )
